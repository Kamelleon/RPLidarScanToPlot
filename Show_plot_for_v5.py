...
def update_plot():
    ...
    while plot_update:
        try:
            ...
            x = pickle.load(open('x_scan_19.pkl', 'rb'))
            distance = pickle.load(open('distance_scan_19.pkl', 'rb'))
            movement = pickle.load(open('movement_scan_19.pkl', 'rb'))
            ...
            plt.scatter(x, movement, s=0.5, c=distance, cmap='coolwarm_r', vmin=min_color_value,
                        vmax=max_color_value)  # Show plot
            ...
            if floor(left_bottom_value) == color_avg or floor(left_bottom_value) == color_avg + 1 or floor(
                ...
                if not printed_warn_left_bottom:
                    playsound(warn_sound, block=False)
